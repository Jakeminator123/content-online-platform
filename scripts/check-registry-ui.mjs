...
try{
  ...
  await row.waitFor();
  await row.getByRole('button',{name:'Styr kund',exact:true}).click();
  ...
  await row.getByRole('link',{name:'Granska kundyta'}).waitFor();
  assert.equal(await row.getByRole('link',{name:'Granska kundyta'}).getAttribute('href'),'https://fokus-psi-sable.vercel.app/o/browser-customer');
