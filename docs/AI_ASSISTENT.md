...
Embed-konfigurationen hämtas från `/admin/api/assistant/presenter` efter befintlig adminbehörighetskontroll. D-ID-scriptet laddas först när användaren klickar **Starta agenten här**. D-ID:s API och Studio-delningslänk använder en Base64-kodad `client_key`; servern accepterar även en rå `ck_…`-form som administrativ indata men validerar och normaliserar alltid till D-ID:s kodade format innan embed-scriptet får den. API-nycklar accepteras aldrig.
