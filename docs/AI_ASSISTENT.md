...
D-ID-agentens `DID_AGENT_ID` och frontendavsedda `DID_CLIENT_KEY` hör till Vercel-projektet `content-online-platform`, som levererar kundsidorna. De är den gemensamma demostandarden för agentaktiverade portaler och sätts som läsbara **Config**-värden för alla miljöer. Admin exponerar bara om standarden är konfigurerad, aldrig själva värdena. En D-ID API key får aldrig läggas i repositoryt, markeras som Config eller skickas till webbläsaren. Saknad eller ogiltig konfiguration döljer widgeten utan att blockera portalen.

Embed-nyckelns allowed domains ska begränsas till den exakta kundportal-origin som faktiskt används. För den delade publicerade adressen är det `https://content-online-platform.vercel.app`; `/portal/{slug}` ska inte ingå eftersom D-ID tillåter origins, inte paths. En egen kunddomän måste också tillåtas exakt eller använda en komplett kundunik agent-ID/client-key-override. Ändringar av Vercels miljövariabler börjar gälla först i en ny deployment.
