...
export const STATISTICS_POLICY_VERSION = '2026-09-05.1';
...
export function selectStatisticsViews(input: {
  ...
  if (declined.length) add('changes', 'Följ upp minskad användning', `${declined.length} produkter har minskat mer än 5 % jämfört med samma period föregående år.`, 100, 'attention');
  ...
  if (upcoming.length) add('renewals', 'Förbered nästa förnyelse', `${upcoming.length} produkter har förnyelsedatum inom 90 dagar. Se användning och datum tillsammans.`, 90, 'attention');
  const growing = rows.filter(r => valid(r.requestsPrevYtd) && r.requestsPrevYtd > 0 && r.requestsYtd! >= r.requestsPrevYtd * 1.1);
  if (!declined.length && growing.length) add('changes', 'Se vad som växer', `${growing.length} produkter har minst 10 % högre användning än samma period föregående år.`, 75, 'opportunity');
