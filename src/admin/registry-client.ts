...
(() => {
  ...
  function render(){
    ...
    let html='<div class="card-head"><div><div class="eyebrow">SPARAT REGISTER · NEON</div><h2>'+(view()==='publishers'?'Content Onlines publicister':view()==='customers'?'Kundsajter & styrning':'Kunder, sajter & publicister')+'</h2><p>Version '+snapshot.version+' · En Vercel-runtime, en Fokus-mall och separat konfiguration per kund.</p></div>'+button('Uppdatera','reload')+'</div>';
  ...
  function customerEditor(customer){
    ...
    const preset='<label>Portal-mall<select class="registry-input" name="preset"><option value="insight" '+(site.preset==='insight'?'selected':'')+'>Fokus · komplett</option><option value="library" '+(site.preset==='library'?'selected':'')+'>Library · kunskapsfokus</option><option value="minimal" '+(site.preset==='minimal'?'selected':'')+'>Minimal · avskalad</option></select></label>';
