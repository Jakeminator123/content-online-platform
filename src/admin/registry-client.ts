...
(() => {
  ...
  function render(){
    ...
    if(view()==='customers'){
      ...
      html+=customers.map(customer=>{
        ...
        const agentState=agentMode==='customer'?'Egen D-ID-konfiguration':agentMode==='platform_fallback'?'Plattformens demoagent':agentMode==='incomplete'?'D-ID saknar giltig standard':'Agent avstängd';
  ...
  function customerEditor(customer){
    ...
    const platformAgentText=agentMode==='customer'?'Egen D-ID-konfiguration är aktiv.':agentMode==='platform_fallback'?'Plattformens demoagent är aktiv; inga kundunika D-ID-värden behövs.':defaultReady?'Plattformens demoagent är redo. Slå på reglaget för att använda den.':'Plattformens demoagent saknar giltig Agent ID/client key och widgeten förblir dold.';
