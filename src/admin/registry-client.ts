...
(() => {
  ...
  function customerEditor(customer){
    ...
    const platformAgentText=agentMode==='customer'?'Egen D-ID-konfiguration är sparad. Testa widgeten på portalens faktiska origin.':agentMode==='platform_fallback'?'Plattformens demoagent är konfigurerad; inga kundunika D-ID-värden behövs. Testa widgeten på portalens faktiska origin.':defaultReady?'Plattformens demoagent är konfigurerad. Slå på reglaget och testa widgeten på portalens faktiska origin.':'Plattformens demoagent saknar giltig Agent ID/client key och widgeten förblir dold.';
