...
(() => {
  ...
  const url=c=>'https://fokus-psi-sable.vercel.app/o/'+encodeURIComponent(c.slug)+'/login';
  ...
  const field=(name,title,value='')=>'<label>'+title+'<input class="registry-input" name="'+name+'" value="'+esc(value)+'" maxlength="120" required></label>';
  function render(){
    ...
    if(view()==='customers'){
      html+='<p>En egen URL per kund. Publicering delar organisationsnamnet och aktiveringssidan, inte användare eller intern data. Inget nytt Vercel-projekt behövs.</p><form data-reg-form="add_customer" class="registry-form">'+field('name','Organisationsnamn')+field('slug','URL-namn (t.ex. universitet-nord)')+'<button class="button teal" type="submit">Lägg till kund</button></form>';
      html+=customers.map(c=>'<div class="list-item"><div class="body"><strong>'+esc(c.name)+'</strong><small>'+esc(label[c.status])+' · '+(c.kind==='demo'?'Syntetisk demo':'Kundkonton inväntar aktivering')+'</small><small>'+esc(url(c))+'</small><small>'+c.publisherIds.map(id=>esc(publishers.find(p=>p.id===id)?.name||id)).join(', ')+'</small></div><div class="registry-actions">'+button('Redigera','edit_customer',c.id)+(c.status==='published'?'<a class="button secondary" target="_blank" rel="noopener noreferrer" href="'+url(c)+'">Öppna kundportal</a>'+button('Avpublicera','unpublish_customer',c.id):c.status==='draft'?button('Publicera','publish_customer',c.id):button('Återställ till utkast','restore_customer',c.id))+(c.status!=='archived'?button('Arkivera','archive_customer',c.id):'')+'</div></div>').join('');
      html+='<div id="registry-editor"></div><p class="footnote">URL-namn återanvänds inte efter arkivering. KTH-konton gäller bara KTH:s syntetiska demo. Nya kunder får ingen KTH-data eller kundåtkomst förrän riktig medlemsinloggning har kopplats in.</p>';
    }
    if(view()==='publishers'){
  ...
  root.addEventListener('click',event=>{
    ...
    if(action==='publish_customer'&&!confirm('Publicera '+item.name+' och dess aktiveringssida på kundens URL? Namnet blir offentligt. Kunddata och konton publiceras inte.'))return;
