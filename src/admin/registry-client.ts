  ...
  function render(){
    ...
    if(view()==='customers'){
      ...
      html+=customers.map(customer=>{
        ...
        const agentState=agentMode==='customer'?'Egen D-ID-konfiguration sparad':agentMode==='platform_fallback'?'Gemensam D-ID-agent konfigurerad':agentMode==='incomplete'?'D-ID saknar giltig standard':'Agent avstängd';
