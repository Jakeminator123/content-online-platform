...
(() => {
  ...
  const swatch=customer=>customer.site.logoUrl?'<span class="registry-swatch registry-logo-mark" aria-label="Kundlogotyp"><img src="'+esc(customer.site.logoUrl)+'" alt="" referrerpolicy="no-referrer"></span>':'<span class="registry-swatch" aria-label="Temafärger"><i style="background:'+esc(customer.site.primaryColor)+'"></i><i style="background:'+esc(customer.site.accentColor)+'"></i></span>';
  const presetLabel=preset=>({insight:'Standard',library:'Library',minimal:'Minimal'}[preset]||'Standard');
  ...
  function overviewView(customers,publishers,members){
    const managedCustomers=customers.filter(customer=>customer.kind==='customer');
    const activeCustomers=managedCustomers.filter(customer=>customer.status!=='archived');
    const pendingDomains=managedCustomers.filter(customer=>customer.site.domain&&!domainReady(customer));
    ...
    return '<div class="registry-overview"><div><strong>'+activeCustomers.length+'</strong><span>Aktiva kunder</span></div><div><strong>'+managedCustomers.filter(customer=>customer.status==='published').length+'</strong><span>Publicerade portaler</span></div><div><strong>'+members.filter(member=>member.status==='active').length+'</strong><span>Aktiva portalbehörigheter</span></div><div><strong>'+managedCustomers.filter(customer=>customer.salesforceAccountId).length+'</strong><span>Salesforce-kopplingar</span></div></div><div class="soft-box"><strong>Nästa att hantera</strong>'+next.map(item=>'<span>'+esc(item)+'</span>').join('')+'</div><p class="section-space"><a class="button teal" href="#customers">Hantera kunder</a> <a class="button secondary" href="#connections">Granska anslutningar</a></p>';
  }

  function customersView(customers,publishers){
    let html='<div class="soft-box"><strong>1. Skapa kundutkast</strong>Välj organisation och URL-namn. Utkastet får ingen publik portal förrän du uttryckligen väljer Publicera.</div><form data-reg-form="add_customer" class="registry-form registry-create">'+field('name','Organisationsnamn')+slugField()+'<button class="button teal" type="submit">Skapa kundutkast</button></form>';
    const matchesCustomer=customer=>matches(customer.name,customer.slug,customer.status,customer.site.domain,customer.publisherIds.map(publisherName).join(' '));
    const visible=customers.filter(customer=>customer.kind==='customer'&&matchesCustomer(customer));
    const demos=customers.filter(customer=>customer.kind==='demo'&&matchesCustomer(customer));
    const customerRow=(customer,demo=false)=>{
      const preview=url(customer),login=customerLoginUrl(customer);
      const portalState=customer.status==='published'?'<small><b>Publicerad portal:</b> '+esc(preview)+'</small>':customer.status==='draft'?'<small><b>Portal:</b> Inte publicerad ännu · reserverad slug '+esc(customer.slug)+'</small>':'<small><b>Portal:</b> Arkiverad och offline</small>';
      const statusActions=demo?'<a class="button secondary" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" href="'+esc(preview)+'">Granska demo</a>':(customer.status==='published'?'<a class="button secondary" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" href="'+esc(preview)+'">Granska kundsajt</a><a class="button secondary" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer" href="'+esc(login)+'">Kundinloggning</a>'+button('Avpublicera','unpublish_customer',customer.id):customer.status==='draft'?button('Publicera','publish_customer',customer.id):button('Återställ till utkast','restore_customer',customer.id))+(customer.status!=='archived'?(customer.status==='draft'?button('Ta bort utkast…','archive_customer',customer.id):button('Arkivera kundsajt','archive_customer',customer.id)):button('Radera permanent','delete_customer',customer.id));
      return '<article class="registry-customer '+(demo?'registry-customer-demo':'')+'"><div class="registry-customer-main">'+swatch(customer)+'<div class="body"><strong>'+esc(customer.name)+(demo?' · Syntetisk pilot':'')+'</strong><small>'+esc(label[customer.status])+' · '+(demo?'Separat referensdemo, inte kunddata':'Portalprofil: '+presetLabel(customer.site.preset))+' · '+esc(agentState(customer))+'</small>'+portalState+'<small><b>Egen domän (valfritt):</b> '+esc(customer.site.domain||'Inte konfigurerad')+(customer.site.domain?' · '+esc(domainState(customer)):'')+'</small><small><b>Publicister:</b> '+esc(customer.publisherIds.map(publisherName).join(', ')||'Inga valda')+'</small></div></div><div class="registry-actions">'+button(demo?'Styr demo':'Styr kundsajt','edit_customer',customer.id,'button teal')+(customer.site.domain&&!domainReady(customer)?button('Kontrollera DNS','ensure_domain',customer.id):'')+statusActions+'</div></article>';
    };
    html+='<div class="registry-list-heading"><div><strong>Riktiga kundsajter</strong><small>'+visible.length+' visas · '+visible.filter(customer=>customer.status==='published').length+' publicerade · '+visible.filter(customer=>customer.status==='draft').length+' utkast</small></div></div>';
    html+=visible.length?visible.map(customer=>customerRow(customer)).join(''):'<div class="empty"><h3>Ingen riktig kund matchar sökningen</h3><p>Prova ett annat namn, URL-namn eller status.</p></div>';
    if(demos.length)html+='<details class="registry-demo section-space" open><summary><strong>Referensdemo – inte en kund ('+demos.length+')</strong></summary><p class="registry-hint">KTH är en skyddad syntetisk presentationspilot och innehåller inga verkliga kunddata. Nya kunder skapas från den gemensamma Standard-mallen, inte genom att kopiera KTH.</p>'+demos.map(customer=>customerRow(customer,true)).join('')+'</details>';
    return html+'<div id="registry-editor"></div><p class="footnote">När ett utkast har skapats öppnas inställningarna. Publicera först när den offentliga portalytan är klar. Ta bort ett felaktigt utkast genom att arkivera det och därefter bekräfta permanent radering med exakt slug.</p>';
  }
  ...
  function customerEditor(customer){
    ...
    const logo='<div class="registry-logo-upload"><div class="registry-logo-preview" data-logo-preview>'+swatch(customer)+'</div><label>Ladda upp logotyp<input class="registry-input" type="file" name="logoFile" accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" aria-describedby="registry-logo-help"><small class="registry-hint" id="registry-logo-help">PNG, JPG eller WebP · högst 2 MB. Filen lagras offentligt för att kunna visas i kundportalen.</small></label></div><details '+(site.logoUrl?'open':'')+'><summary>Alternativ: använd befintlig publik bildadress</summary>'+optionalField('logoUrl','Publik HTTPS-adress',site.logoUrl,500,'url','Lämna tomt för kundens initialer')+'</details>';
    ...
    const publication=customer.kind==='demo'?'<div class="soft-box"><strong>Skyddad syntetisk pilot</strong>KTH hålls separat från riktiga kunder och kan granskas som presentationsdemo.</div>':customer.status==='draft'?'<div class="soft-box"><strong>2. Anpassa och publicera</strong>Portalen är inte offentlig ännu. Spara önskade inställningar och välj sedan Publicera portal. '+button('Publicera portal','publish_customer',customer.id,'button teal')+'</div>':customer.status==='published'?'<div class="soft-box"><strong>Portalen är publicerad</strong><a href="'+esc(url(customer))+'" target="_blank" rel="noopener noreferrer" referrerpolicy="no-referrer">Öppna publicerad portal</a></div>':'<div class="soft-box"><strong>Kundsajten är arkiverad</strong>Återställ den till utkast innan den kan ändras eller publiceras igen. '+button('Återställ till utkast','restore_customer',customer.id)+'</div>';
    return '<section class="registry-editor-card"><div class="registry-editor-head"><div><div class="eyebrow">STYR KUNDSAJT</div><h3>'+esc(customer.name)+'</h3><p>'+(customer.status==='published'?esc(url(customer)):customer.status==='draft'?'Utkast · ingen publik portal ännu':'Arkiverad · portalen är offline')+'</p></div>'+button('Stäng','close_editor')+'</div>'+publication+'<div class="soft-box"><strong>URL-namn</strong><span>'+esc(customer.slug)+'</span><small>Den publika adressen blir '+esc(fallbackUrl(customer))+' först efter publicering.</small></div><form class="registry-form registry-editor-form" data-reg-form="update_customer" data-id="'+esc(customer.id)+'"><h4>Organisation & publicister</h4>'+field('name','Organisationsnamn',customer.name)+'<fieldset><legend>Publicister i kundens portal</legend>'+publishers.map(publisher=>'<label class="registry-check"><input type="checkbox" name="publisherIds" value="'+esc(publisher.id)+'" '+(customer.publisherIds.includes(publisher.id)?'checked':'')+'>'+esc(publisher.name)+(publisher.status==='archived'?' (arkiverad)':'')+'</label>').join('')+'</fieldset><button class="button teal" type="submit">Spara organisation</button></form><form class="registry-form registry-editor-form" data-reg-form="configure_customer_site" data-id="'+esc(customer.id)+'"><h4>Varumärke & portal</h4>'+preset+logo+colors+field('heading','Portalrubrik',site.heading,120)+field('tagline','Ingress',site.tagline,240)+customDomain+'<fieldset class="registry-agent"><legend>D-ID-agent för '+esc(customer.name)+'</legend><label class="registry-switch"><input type="checkbox" name="agentEnabled" '+(agent.enabled?'checked':'')+'> Visa agenten på kundens portal</label><p class="registry-hint"><strong>'+esc(platformAgentText)+'</strong></p>'+agentOverride+field('greeting','Agentens hälsning',agent.greeting,240)+'<label>Positivitet: <output data-positivity-output>'+agent.positivity+'</output>/10<input class="registry-input registry-range" type="range" name="positivity" min="1" max="10" step="1" value="'+agent.positivity+'"></label><p class="registry-hint">Nivån styr ton, aldrig fakta. Agenten får inte dölja kostnader, nedgångar eller osäkerhet.</p><div class="registry-tools"><strong>Tillåtna klientverktyg</strong>'+tools+'</div></fieldset><button class="button teal" type="submit">Spara kundsajt</button></form>'+portalMembersEditor(customer)+'</section>';
  }
  ...
  function portalMembersEditor(customer){
    ...
    const heading='<div class="registry-form registry-editor-form" aria-labelledby="portal-members-heading"><h4 id="portal-members-heading">Inloggning och åtkomst</h4><p class="registry-hint">Clerk verifierar ett gemensamt Content Online-konto. Neonregistret avgör sedan exakt vilka kundportaler kontot får öppna. För en publicerad portal skickas inbjudan automatiskt när du lägger till användaren.</p></div>';
    const existing=members.length?members.map(member=>{const binding=member.identityBound?'Kontot är verifierat och kopplat.':customer.status!=='published'?'Publicera portalen innan en inbjudan skickas.':'Väntar på att inbjudan ska accepteras.';const invite=customer.status==='published'&&member.status==='active'&&!member.identityBound?button('Skicka inbjudan igen','invite_portal_member',member.id):'';return '<form class="registry-form registry-editor-form" data-reg-form="update_portal_member" data-id="'+esc(member.id)+'" aria-label="Redigera portalanvändare '+esc(member.displayName)+'"><h4>'+esc(member.displayName)+' · '+esc(label[member.status])+'</h4><p class="registry-hint">'+esc(binding)+'</p>'+field('verifiedEmail','Verifierad e-postadress',member.verifiedEmail,254,'email')+field('displayName','Visningsnamn',member.displayName)+roleField(member.role)+'<label>Status<select class="registry-input" name="status">'+memberStatusOptions(member.status)+'</select></label><div class="registry-actions"><button class="button teal" type="submit">Spara portalanvändare</button>'+invite+'</div></form>';}).join(''):'<p class="registry-hint">Inga portalbehörigheter har lagts till ännu.</p>';
    const add='<form class="registry-form registry-editor-form" data-reg-form="add_portal_member" data-customer-id="'+esc(customer.id)+'"><h4>Lägg till och bjud in</h4>'+field('verifiedEmail','Verifierad e-postadress','',254,'email','Adressen blir kontots identitet och måste matcha exakt vid inloggning.')+field('displayName','Visningsnamn')+roleField('customer_reader')+'<button class="button teal" type="submit">Lägg till och skicka inbjudan</button></form>';
  ...
  async function uploadLogo(customerId,file){
    const token=await adminToken();
    const form=new FormData();form.set('version',String(snapshot.version));form.set('logo',file);
    const response=await fetch('/admin/api/customers/'+encodeURIComponent(customerId)+'/logo',{method:'POST',credentials:'omit',cache:'no-store',headers:{Authorization:'Bearer '+token},body:form});
    if(!response.ok){
      if(response.status===413)throw new Error('Logotypen är större än 2 MB. Välj en mindre bild.');
      if(response.status===415)throw new Error('Logotypen måste vara en riktig PNG-, JPG- eller WebP-bild.');
      if(response.status===409)throw new Error('Kundposten har ändrats. Uppdatera listan och välj logotypen igen.');
      throw new Error('Logotypen kunde inte laddas upp. Ingen ny bildadress har sparats.');
    }
    const result=await response.json();
    if(typeof result.url!=='string'||!result.url.startsWith('https://'))throw new Error('Logotyptjänsten svarade utan en giltig bildadress.');
    return result.url;
  }
  ...
  async function mutate(command){
    ...
    try{snapshot=await request({version:snapshot.version,command});render();if(command.action==='add_customer'){const customer=snapshot.data.customers.find(item=>item.slug===command.slug);const editor=document.getElementById('registry-editor');if(customer&&editor){editor.innerHTML=customerEditor(customer);editor.scrollIntoView({block:'start',behavior:'smooth'});}state('Kundutkastet är skapat. Anpassa det och välj Publicera portal när det är klart.');}else if(command.action==='add_portal_member'&&snapshot.invitation?.status==='sent')state('Portalbehörigheten är sparad och inbjudan är skickad.');else if(command.action==='add_portal_member'&&snapshot.invitation?.reason==='customer_not_published')state('Portalbehörigheten är sparad. Publicera portalen innan du skickar inbjudan.');else if(command.action==='add_portal_member')state('Portalbehörigheten är sparad, men inbjudan kunde inte skickas. Använd Skicka inbjudan igen.');else if(command.action==='publish_customer')state('Kundportalen är publicerad och kan nu granskas.');else if(command.action==='archive_customer')state('Posten är arkiverad. Välj Radera permanent om den ska tas bort helt.');else state('Ändringen är sparad.');}catch(error){state(error.message);}finally{busy=false;root.querySelectorAll('button').forEach(node=>node.disabled=false);}
  }
  async function invitePortalMember(id){
    if(busy||!snapshot)return;busy=true;root.querySelectorAll('button').forEach(node=>node.disabled=true);state('Skickar inbjudan…');
    try{const token=await adminToken();const response=await fetch('/admin/api/portal-members/'+encodeURIComponent(id)+'/invite',{method:'POST',credentials:'omit',cache:'no-store',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify({version:snapshot.version})});if(!response.ok){if(response.status===409)throw new Error('Inbjudan kan bara skickas till en aktiv användare i en publicerad portal som ännu inte har kopplat sitt konto.');throw new Error('Inbjudan kunde inte skickas. Behörigheten i Neon är oförändrad.');}state('Inbjudan är skickad till den verifierade e-postadressen.');}catch(error){state(error.message);}finally{busy=false;root.querySelectorAll('button').forEach(node=>node.disabled=false);}
  }
  ...
  root.addEventListener('click',event=>{
    ...
    if(action==='invite_portal_member'){invitePortalMember(id);return;}
    ...
    if(action==='archive_customer'&&!confirm(item.status==='draft'?'Arkivera utkastet för '+item.name+'? Efteråt kan du radera det permanent genom att bekräfta exakt slug.':'Arkivera kundsajten för '+item.name+'? Portalen blir omedelbart otillgänglig, men kundpost och inställningar bevaras så att sajten kan återställas.'))return;
  ...
  root.addEventListener('input',event=>{
    ...
    if(event.target instanceof HTMLInputElement&&event.target.name==='logoFile'&&event.target.files?.[0]){const preview=event.target.closest('form')?.querySelector('[data-logo-preview]');if(preview){const image=document.createElement('img');image.src=URL.createObjectURL(event.target.files[0]);image.alt='Förhandsvisning av vald logotyp';preview.replaceChildren(image);}}
  ...
  root.addEventListener('submit',async event=>{
    ...
    if(action==='configure_customer_site'){
      let logoUrl=String(data.get('logoUrl')||'').trim();const logoFile=data.get('logoFile');
      if(logoFile instanceof File&&logoFile.size){if(busy||!snapshot)return;busy=true;root.querySelectorAll('button').forEach(node=>node.disabled=true);state('Laddar upp logotyp…');try{logoUrl=await uploadLogo(form.dataset.id,logoFile);}catch(error){state(error.message);return;}finally{busy=false;root.querySelectorAll('button').forEach(node=>node.disabled=false);}}
      mutate({action,id:form.dataset.id,site:{preset:String(data.get('preset')||'insight'),domain:String(data.get('domain')||'').trim().toLowerCase(),logoUrl,primaryColor:String(data.get('primaryColor')||''),accentColor:String(data.get('accentColor')||''),heading:String(data.get('heading')||'').trim(),tagline:String(data.get('tagline')||'').trim(),agent:{enabled:data.get('agentEnabled')==='on',agentId:String(data.get('agentId')||'').trim(),clientKey:String(data.get('clientKey')||'').trim(),greeting:String(data.get('greeting')||'').trim(),positivity:Number(data.get('positivity')||5),tools:data.getAll('agentTools')}}});return;
    }
