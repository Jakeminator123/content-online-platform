...
(() => {
  ...
  function customersView(customers,publishers){
    ...
    const customerRow=(customer,demo=false)=>{
      ...
      return '<article class="registry-customer"><div class="registry-customer-main">'+swatch(customer)+'<div class="body"><strong>'+esc(customer.name)+(demo?' · Syntetisk pilot':'')+'</strong><small>'+esc(label[customer.status])+' · '+(demo?'Visningsdemo':'Aktuell standarddashboard')+' · '+esc(agentState(customer))+'</small>'+portalState+'<small><b>Egen domän (valfritt):</b> '+esc(customer.site.domain||'Inte konfigurerad')+(customer.site.domain?' · '+esc(domainState(customer)):'')+'</small><small><b>Publicister:</b> '+esc(customer.publisherIds.map(publisherName).join(', ')||'Inga valda')+'</small></div></div><div class="registry-actions">'+button(demo?'Styr demo':'Styr kundsajt','edit_customer',customer.id,'button teal')+(customer.site.domain&&!domainReady(customer)?button('Kontrollera DNS','ensure_domain',customer.id):'')+statusActions+'</div></article>';
    };
    ...
    if(demos.length)html+='<details class="section-space"><summary><strong>Syntetisk visningsdemo ('+demos.length+')</strong></summary><p class="registry-hint">KTH är en skyddad presentationspilot, inte en kundorganisation. Den räknas inte i kundöversikten och kan inte raderas från kundflödet.</p>'+demos.map(customer=>customerRow(customer,true)).join('')+'</details>';
