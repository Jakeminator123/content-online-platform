...
export const adminJobs = [
  {
    id: 'platform-readiness', title: 'Plattformens beredskap',
    description: 'Kontrollerar lagring, datakällor och pilotstatus samt väljer relevanta statistikvyer per kund. Inga externa skrivningar.',
    schedule: 'Dagligen 06:10 UTC', trigger: 'scheduled_and_manual',
  },
  {
    id: 'customer-scope-audit', title: 'Kund- och rollöversikt',
    description: 'Summerar syntetiska kunder och konton. Detta är inte en säkerhetsgranskning av faktisk åtkomst.',
    schedule: 'Manuell', trigger: 'manual',
  },
  {
    id: 'renewal-preflight', title: 'Förnyelseunderlag',
...
export type JobExecution = {
  ...
  startedAt: string; finishedAt: string; summary: string;
  facts: Array<{ label: string; value: string }>;
  statistics?: ReturnType<typeof buildWorkspaceStatistics>;
  persisted: false;
};
export function runAdminJob(jobId: string, workspace: DemoWorkspace, now = new Date()): JobExecution | null {
  ...
  if (jobId === 'platform-readiness') {
    const statistics = buildWorkspaceStatistics(workspace, now);
    return {
      jobId,
      status: workspace.storage.status === 'blocked_by_decision' || connectedSources === 0 ? 'attention_needed' : 'completed',
      mode: 'synthetic_read_only', startedAt: timestamp, finishedAt: timestamp,
      summary: `${connectedSources} av ${workspace.connections.length} källor är anslutna. ${workspace.storage.label}. Statistikfokus har beräknats för kundernas befintliga underlag.`,
      facts: [
        ...
        ...statistics.customers.flatMap(customer => customer.focus.recommendations.length
          ? customer.focus.recommendations.map(item => ({ label: `${customer.customerName} · ${item.title}`, value: item.reason }))
          : [{ label: `${customer.customerName} · statistik`, value: 'Kundspecifikt underlag saknas. Inga mätvärden har antagits.' }]),
        { label: 'Urvalsmetod', value: 'Förklarbara regler. Inget AI-anrop, ingen ändring av kundens behörighet eller sparad körhistorik.' },
      ],
      statistics, persisted: false,
    };
  }
  if (jobId === 'customer-scope-audit') {
    return {
      jobId, status: 'completed', mode: 'synthetic_read_only', startedAt: timestamp, finishedAt: timestamp,
  ...
  return {
    jobId: 'renewal-preflight', status: 'attention_needed', mode: 'synthetic_read_only',
