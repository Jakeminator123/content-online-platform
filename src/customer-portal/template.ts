...
function portalPage(customer: RegistryCustomer, registry: Registry, options: CustomerPortalRenderOptions): string {
  ...
  <section class="portal-section" data-portal-section="products" data-portal-label="Produkter" hidden><header class="section-intro compact"><div><span class="section-kicker">PORTFÖLJ</span><h1>Era produkter</h1><p>${isDemo ? `${products.length} tilldelade informationsprodukter.` : "Tilldelningar visas efter säker inloggning."}</p></div></header><section class="surface"><div class="surface-head"><h2>Informationsprodukter</h2>${isDemo ? `<span class="status-chip">${products.length} produkter</span>` : ""}</div>${productRows(customer, registry)}</section></section>
  <section class="portal-section" data-portal-section="usage" data-portal-label="Analys" hidden><header class="section-intro compact"><div><span class="section-kicker">ANALYS</span><h1>Användning och beteende</h1><p>${isDemo ? "Utforska trender, produktanvändning och fördelning över portföljen." : "Analysen kräver verifierade, tenant-avgränsade källor."}</p></div></header>${insights ? renderInsightsPanel(insights) : lockedState("Analys öppnas när medlemskap och datakällor är aktiverade.")}</section>
