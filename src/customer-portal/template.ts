...
function portalPage(customer: RegistryCustomer, registry: Registry, options: CustomerPortalRenderOptions): string {
  ...
  <section class="portal-section" data-portal-section="configuration" data-portal-label="Konfiguration" hidden><header class="section-intro compact"><div><span class="section-kicker">KONFIGURATION</span><h1>Portal och ärenden</h1><p>${isDemo ? "Anpassa demovyn och förhandsvisa en spårbar begäran till Content Online." : "Inställningar och ärenden kräver verifierat medlemskap."}</p></div></header>${renderConfigurationPanel(isDemo)}</section>
