...
const periods: Record<TelemetryPeriodId, TelemetryPeriod> = {
  "30d": createPeriod(
    "30 dagar",
    "femdagarsvärden · jämfört med föregående 30 dagar",
  ...
  "12m": createPeriod(
    "12 månader",
    "månadsvärden · jämfört med föregående 12 månader",
...
export function renderOverviewMetrics(insights: PortalInsightsConfig): string {
  const values = [
    ...
    [String(insights.productCount), "Produkter", `${insights.publisherCount} publicister`],
  ];
  return `<div class="metric-grid metric-grid-primary">${values.map(([value, label, description]) => `<article class="metric-card"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong><small>${escapeHtml(description)}</small></article>`).join("")}</div>`;
}
