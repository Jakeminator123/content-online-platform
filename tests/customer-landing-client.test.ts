...
describe("customer landing login overlay", () => {
  ...
  it("opens a legacy portal preference in the canonical overlay and clears it on close", () => {
    const root = new FakeElement();
    const body = new FakeElement();
    const menu = new FakeElement();
    const menuButton = new FakeElement();
    ...
    new Script(customerLandingClient).runInNewContext({
      ...
      IntersectionObserver: undefined,
      document,
      location: { href: "https://content-online-platform.vercel.app/?portal=north" },
      performance: { now: () => 0 },
      window,
    });
    ...
    expect(dialog.open).toBe(true);
    expect(access.dataset.customerAccessRequested).toBe("true");
    dialog.listeners.get("close")?.();
