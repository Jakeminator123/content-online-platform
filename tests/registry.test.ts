...
describe("Neon persistence adapter", () => {
  ...
  it("keeps credentials on approved Neon HTTPS host, disallows redirects and redacts provider failures", async () => {
    ...
    const query = neonQuery("postgresql://fixture:fake@ep-test.eu-central-1.aws.neon.tech/db", fetcher);
    ...
    for (const connection of ["", "http://example.com", "postgresql://a:b@evil.neon.tech.example/db", "postgresql://a:b@localhost/db"]) expect(() => neonQuery(connection)).toThrow("storage_unconfigured");
    const broken = neonQuery("postgresql://fixture:fake@ep-test.eu-central-1.aws.neon.tech/db", async () => new Response("secret provider details", { status: 400 }));
